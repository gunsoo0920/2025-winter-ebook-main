...
const MainPage = () => {
  ...
  useEffect(() => {
    ...
    const weekRanges = [
      { start: new Date(2026, 0, 1), end: new Date(2026, 0, 11, 23, 59, 59) }, // 1주차
      { start: new Date(2026, 0, 12), end: new Date(2026, 0, 18, 23, 59, 59) }, // 2주차
      { start: new Date(2026, 0, 19), end: new Date(2026, 0, 25, 23, 59, 59) }, // 3주차
      { start: new Date(2026, 0, 26), end: new Date(2026, 1, 1, 23, 59, 59) }, // 4주차
    ];
    ...
    allData.forEach((item) => {
      ...
      const returnedDate = item.returnedDate
        ? parseDate(item.returnedDate)
        : null;
      ...
      if (
        returnedDate &&
        lendingDate.toDateString() === returnedDate.toDateString()
      )
        return;
      ...
      if (lendingDate >= eventStartDate && lendingDate <= eventEndDate) {
        ...
        for (let i = 0; i < weekRanges.length; i++) {
          ...
          if (
            lendingDate >= weekRanges[i].start &&
            lendingDate <= weekRanges[i].end
          ) {
            counts[i]++;
